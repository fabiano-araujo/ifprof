...
public class ShowDisciplinas extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{

    private Button btnCriarDisciplina;
    private Toolbar tbShowDisciplinas;
    private RecyclerView rvDisciplina;
    private List<AllInfo> mList;
    private Repositorio repositorio;
    private CoordinatorLayout cdlShowDisciplinas;
    private ScrollView svShowDisciplinas;
    private FloatingActionButton FABAddDisciplina;
    private NavigationView nvShowDisciplinas;
    private DrawerLayout dlShowDisciplinas;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCriarDisciplina.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivity(new Intent(ShowDisciplinas.this, ActivityAddDisciplinas.class));
                ShowDisciplinas.this.finish();
            }
        });
        try{
        ...
        View view = AlertsAndControl.configurationNavigationView(this,dlShowDisciplinas,getLayoutInflater(), professorLogged);
        ivNavigation =(ImageView) view.findViewById(R.id.ivHeader);
        nvShowDisciplinas.addHeaderView(view);
        toggle = new ActionBarDrawerToggle(this,dlShowDisciplinas, tbShowDisciplinas,R.string.drawer_open,R.string.drawer_close);
        dlShowDisciplinas.setDrawerListener(toggle);
        toggle.syncState();

        if (mList.size() < 1){
            rvDisciplina.setVisibility(View.GONE);
            ...
            rvDisciplina.setLayoutManager(llm);
            FABAddDisciplina.setVisibility(View.VISIBLE);
            svShowDisciplinas.setVisibility(View.GONE);
            adapterInfo = new AdapterInfo(this,mList,AdapterInfo.SHOWDISCIPLINAS,svShowDisciplinas,rvDisciplina,FABAddDisciplina);
            rvDisciplina.setAdapter(adapterInfo);
        }
        ...
        if (getIntent().getStringExtra("insert") != null){
            if (getIntent().getStringExtra("insert").equals("ok")){
                Snackbar.make(cdlShowDisciplinas, "Inserido com sucesso!", Snackbar.LENGTH_LONG).show();
            }else if(getIntent().getStringExtra("insert").equals("alter")){
                Snackbar.make(cdlShowDisciplinas, "Alterado com sucesso!", Snackbar.LENGTH_LONG).show();
            }
        }
        ...
        nvShowDisciplinas.getMenu().findItem(R.id.itmDisciplinas).setChecked(true);
    }
    @Override
    public boolean onNavigationItemSelected(MenuItem menuItem) {
        int id = menuItem.getItemId();
        menuItem.setChecked(true);
        AlertsAndControl.navigationViewItemSelectedListener(this,dlShowDisciplinas,id);

        return true;
    }
    @Override
    public void onBackPressed() {
        if (dlShowDisciplinas.isDrawerOpen(GravityCompat.START)) {
            dlShowDisciplinas.closeDrawer(GravityCompat.START);
        } else {
            super.onBackPressed();
        }
    }
    ...
    public void instance(){
        cdlShowDisciplinas = (CoordinatorLayout)findViewById(R.id.cdlShow);
        svShowDisciplinas = (ScrollView)findViewById(R.id.svShow);
        tbShowDisciplinas = (Toolbar)findViewById(R.id.tbShow);
        setSupportActionBar(tbShowDisciplinas);
        FABAddDisciplina = (FloatingActionButton)findViewById(R.id.FABAdd);
        btnCriarDisciplina = (Button)findViewById(R.id.btnCriar);
        dlShowDisciplinas = (DrawerLayout)findViewById(R.id.dlShow);
        nvShowDisciplinas = (NavigationView)findViewById(R.id.nvShow);
        nvShowDisciplinas.setNavigationItemSelectedListener(this);
        rvDisciplina = (RecyclerView)findViewById(R.id.rvShow);
